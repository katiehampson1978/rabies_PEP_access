...
country_horizon_iter_univar <- function(model_output){

    sums = model_output %>%
      dplyr::group_by(country, scenario, variable, iter) %>%
...
# Assign an order to the iterations so we can pick the median and CIs for the analysis
...
############################
# univar_country_summary #
############################
# FOR COUNTRIES summarize over entire time horizon or by year
univar_country_summary = function(model_output, year){

  if(year==TRUE){arg_setting = list(quo(year), quo(country), quo(variable), quo(scenario))}
  if(year==FALSE){arg_setting = list(quo(country), quo(variable), quo(scenario))}
    ...
    dplyr::summarise(
      ...
      U5_deaths_averted = mean(U5_deaths_averted),
      ...
      U5_YLL_averted = mean(YLL_averted_U5),
...
univar_summary = function(model_output, year, setting){
  ...
  if(year==TRUE & setting == "cluster"){arg_setting = list(quo(year), quo(cluster), quo(variable), quo(scenario))}
  if(year==FALSE & setting == "cluster"){arg_setting = list(quo(cluster), quo(variable), quo(scenario))}
